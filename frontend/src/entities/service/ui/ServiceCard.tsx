    ...
    >
      ...
      <div className="p-4 flex flex-col gap-2 flex-1">
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">{service.name}</h3>
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-xs text-gray-400">{service.rating.toFixed(1)}</span>
        </div>
        <p className="font-semibold text-indigo-600 mt-auto">от ${service.priceFrom}</p>
      </div>
    </Link>
  )
}
