...
function MobileNavLink({ to, label, icon: Icon, exact, onClick }: NavItem & { onClick: () => void }) {
  const { pathname } = useLocation()
  const active = exact
    ? pathname === to
    : pathname === to || pathname.startsWith(to + '/')
    ...
    <div ref={ref} className="relative">
      ...
      >
        {user.avatarUrl ? (
          ...
          <div className="w-6 h-6 rounded-full bg-gradient-to-br from-gold to-gold-dark flex items-center justify-center text-[9px] font-bold text-navy shrink-0">
            {initials}
          </div>
        )}
        ...
        <ChevronDown className={cn('size-3 text-muted-foreground/40 transition-transform', open && 'rotate-180')} />
      </button>
...
function AppNavHeader() {
  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-40 w-full border-b border-border/60 bg-background/95 backdrop-blur-[12px]">
        <div className="h-[1.5px] bg-gradient-to-r from-transparent via-gold/50 to-transparent" />

        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 h-[58px] flex items-center gap-4">

          {/* Logo */}
          <Link to="/events" className="flex items-center gap-2 shrink-0 mr-2">
          ...
          {/* Desktop nav */}
          <nav className="hidden md:flex items-stretch h-full gap-5 flex-1">
            {BROWSE_LINKS.map(link => <DesktopNavLink key={link.to} {...link} />)}

            {roleLinks.length > 0 && (
              <div className="w-px bg-border/50 my-4 mx-1" />
            )}

            {roleLinks.map(link => <DesktopNavLink key={link.to} {...link} />)}
          </nav>
        ...
        {mobileOpen && (
          <div className="mob-menu md:hidden border-t border-border/60 bg-background px-4 py-3 flex flex-col gap-0.5">
            {BROWSE_LINKS.map(link => <MobileNavLink key={link.to} {...link} onClick={closeMobile} />)}

            {roleLinks.length > 0 && (
              <>
                <div className="h-px bg-border/40 my-1.5" />
                {roleLinks.map(link => <MobileNavLink key={link.to} {...link} onClick={closeMobile} />)}
              </>
            )}
